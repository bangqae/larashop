...
class OrderController extends Controller
{
	...
	{
		$order = Order::withTrashed()->findOrFail($id); // Tanpa menggunakan withTrashed(), order yang sudah di-remove tidak akan ketemu
	...
	public function destroy($id)
	{
		...
		if ($order->trashed()) {
			...
			if ($canDestroy) {
				...
				\Session::flash('error', 'The order could not be removed permanently');
			}
			...
			$canDestroy = \DB::transaction(
				function () use ($order) {
					if (!$order->isCancelled()) { // Jika status tidak cancelled
						foreach ($order->orderItems as $item) {
							ProductInventory::increaseStock($item->product_id, $item->qty); // Kembalikan stock-nya
						}
					}; // Jika status order cancelled, maka proses di atas dilewati karena di doCancel sudah dikembalikan stock-nya
					// Setelah itu, hapus order
					$order->delete();
			...
			if ($canDestroy) {
				...
				\Session::flash('error', 'The order could not be removed');
			}
